...
function publicLobby(l) {
  return {code:l.code,host:l.host,players:l.players,maxPlayers:l.maxPlayers,status:l.status,gameId:l.gameId||null};
}
...
function createGame(players,lobbyCode) {
  ...
  return store.games[gameId]={
    ...
    gameOver:false,winner:null,lobbyCode,lastAction:Date.now()
  };
}
...
module.exports = async function handler(req,res){
  ...
  if(req.method==='GET'){
    if(action==='lobby'){const l=store.lobbies[req.query.code];return l?ok(res,{lobby:publicLobby(l)}):fail(res,'LOBBY NOT FOUND',404);}
    if(action==='game'){const g=store.games[req.query.gameId];return g?ok(res,{state:publicState(g)}):fail(res,'GAME NOT FOUND',404);}
  ...
  if(req.method==='POST'){
    ...
    try{
      ...
      else{const raw=await new Promise((resolve,reject)=>{let d='';req.on('data',c=>{d+=c;});req.on('end',()=>resolve(d));req.on('error',reject);});body=JSON.parse(raw||'{}');}
    }catch{body={};}

    if(action==='register'){const n=(body.username||'').trim().slice(0,14);if(!n)return fail(res,'INVALID USERNAME');return ok(res,{username:n});}

    if(action==='create_lobby'){
      const{username,maxPlayers}=body;if(!username)return fail(res,'NO USERNAME');
    ...
    if(action==='join_lobby'){
      const{username,code}=body;if(!username)return fail(res,'NO USERNAME');
      const l=store.lobbies[code];if(!l)return fail(res,'LOBBY NOT FOUND');
      if(l.status!=='waiting')return fail(res,'GAME ALREADY STARTED');
      if(!l.players.includes(username)){if(l.players.length>=l.maxPlayers)return fail(res,'LOBBY FULL');l.players.push(username);}
      l.lastActivity=Date.now();return ok(res,{lobby:publicLobby(l)});
    }

    if(action==='leave_lobby'){
      const{username,code}=body;const l=store.lobbies[code];if(!l)return ok(res,{ok:true});
      l.players=l.players.filter(p=>p!==username);
      if(l.players.length===0){delete store.lobbies[code];}else{if(l.host===username)l.host=l.players[0];l.lastActivity=Date.now();}
      return ok(res,{ok:true});
    }

    if(action==='start_game'){
      const{username,code}=body;const l=store.lobbies[code];
      if(!l)return fail(res,'LOBBY NOT FOUND');if(l.host!==username)return fail(res,'ONLY HOST CAN START');
      if(l.players.length<2)return fail(res,'NEED AT LEAST 2 PLAYERS');
      l.status='in_game';const g=createGame(l.players,code);l.gameId=g.gameId;
      return ok(res,{gameId:g.gameId,state:publicState(g)});
    }

    if(action==='rps_choice'){
      const{gameId,username,choice}=body;const g=store.games[gameId];
      ...
      if(g.alivePlayers.every(p=>g.pendingRPS[p])){
        ...
        if(winners.length>1){g.moves.unshift(`🤝 TIE — ${choiceStr}`);g.pendingRPS={};}
        else{const w=winners[0];g.moves.unshift(`⚔ ${w} WON RPS (${choiceStr})`);g.scores[w]+=10;g.rpsWinner=w;g.phase='action';g.pendingRPS={};}
      }
    ...
    if(action==='action_choice'){
      ...
      const target=(targetPlayer&&alive.includes(targetPlayer)&&targetPlayer!==username)?targetPlayer:alive.find(p=>p!==username);
      const me=g.islands[username];
      const QNAMES=['TOP-LEFT','TOP-RIGHT','BOT-LEFT','BOT-RIGHT'];

      if(act==='shield'){
        if(me.shields>=3)return fail(res,'MAX 3 SHIELDS');
        me.shields++;
        ...
        if(!target)return fail(res,'NO TARGET');
        if(useInventory){if(me.inventory.cannons<=0)return fail(res,'NO CANNONS IN INVENTORY');me.inventory.cannons--;}
        ...
        if(!useInventory){me.inventory.cannons++;g.moves.unshift(`📦 ${username} stored a 💣 Cannon`);}
      } else if(act==='slicer'){
        if(!target)return fail(res,'NO TARGET');
        if(useInventory){if(me.inventory.slicers<=0)return fail(res,'NO SLICERS IN INVENTORY');me.inventory.slicers--;}
        const t=g.islands[target];
        if(t.shields>0){
          ...
          if(!useInventory){me.inventory.slicers++;g.moves.unshift(`📦 ${username} stored a ⚔ Slicer`);}
        }
      } else if(act==='store'){
        me.inventory.cannons++;me.inventory.slicers++;
      ...
      if(g.alivePlayers.length===1){
        ...
        return ok(res,{gameOver:true,winner:gw,state:publicState(g)});
      }
